...
public class Database implements DataPersistenceInterface {
    ...
    public List<Group> findGroupByName (String name) {
        ...
        try {
            String sql = "SELECT T1.ID, T1.OWNER, T1.NOME, T1.ATIVO, T2.ID, T2.USER "
                    ...
                    + "WHERE NOME = ?) AS T1 "
                    + "JOIN (SELECT ID, USER FROM USER) AS T2 "
                    + "ON T1.OWNER = T2.ID";
    ...
    public List<Group> findGroupByOwnerUserName (String user) {
        ...
        try {
            String sql = "SELECT T1.ID, T1.OWNER, T1.NOME, T1.ATIVO, T2.ID, T2.USER "
                    + "FROM (SELECT ID, OWNER, NOME, ATIVO FROM GRUPO) AS T1 "
                    + "JOIN (SELECT ID, USER FROM USER WHERE USER = ?) AS T2 "
                    + "ON T1.OWNER = T2.ID";
    ...
    public void groupResults(ResultSet rs, List<Group> list) throws SQLException {
        while(rs.next()) {
            int id = rs.getInt("T1.ID");
            String owner = rs.getString("T2.USER");
            ...
            Group group = new Group(null, nome, id);
            list.add(group);
        }
    }
    ...
    public List<Group> listUserGroups(User user, boolean active) {
        ...
        try {
            String sql = "SELECT ID, OWNER, NOME, ATIVO "
                    + "FROM GRUPO "
                    + "WHERE ID IN (SELECT GRUPO_ID FROM GRUPO_MEMBRO "
                    + "WHERE USER_ID IN (SELECT ID FROM USER WHERE USER = ?)) AND ATIVO = ?";
